// Discover a current record with four observed stats and validate all four positions.
// Each returned V3 leaf is covered exactly once by an exact-value predicate using the multiproof endpoint.
...
import { validateV3Exact } from "../common/score-validation";

async function main(): Promise<void> {
  validateScoreOverrides(4);
  ...
  const name = "V3 four-stat example";
  ...
  const selection = await discoverScoreRecord(users.apiClient, 4);
  selection.statKeys = selection.statKeys.slice(0, 4);
  await validateV3Exact(userProgram, users.apiClient, selection);
...
main().then(
  ...
  error => {
    ...
    console.error(error instanceof Error ? error.message : "Four-stat V3 example failed");
    process.exit(1);
  },
);

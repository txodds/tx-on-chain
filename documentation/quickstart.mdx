...
Subscribe to the TxLINE by paying with TxL. Choose between the free tier, custom league selection, or full access for real-time odds and scores.
...
<Tabs>
  ...
  <Tab title="Full Access">
    ```typescript
    const DURATION_WEEKS = 4; // Minimum 4 weeks

    const txSig = await program.methods
      .subscribeV2(DURATION_WEEKS)
      .accounts({
        user: user.publicKey,
        tokenMint: tokenMint,
        userTokenAccount: userTokenAccount.address,

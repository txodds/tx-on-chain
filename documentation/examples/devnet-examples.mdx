...
<Info>
The scripts use the devnet IDL and generated types in `examples/devnet/idl/txoracle.json` and `examples/devnet/types/txoracle.ts`, updated to v1.5.6 with `validateStatV3` support. For mainnet integrations, use the root `idl/txoracle.json` and `types/txoracle.ts`, which include the latest mainnet program types.
</Info>

## Requirements

- Node.js `20.18` or newer. The current lockfile includes Solana codec packages that require `node >=20.18.0`.
...
<Warning>
The scripts create `<ANCHOR_WALLET>.txline-subscription-recovery.json` before broadcasting a subscription. This sidecar contains only public transaction and network context. It remains after activation as a tombstone: on every restart the scripts validate it, query the saved signature, and refuse to submit a second subscription. Delete it only as an explicit decision to permit a new on-chain subscription. Use `TXLINE_RECOVERY_FILE` only when you need an explicit sidecar location.
</Warning>
...
| `subscription_scores_v3c.ts` | V3 multiproof validation with compressed proofs using `validateStatV3`, exercising 2-, 3-, and 4-leg strategies (v1.5.6+). |
...
There are three score validation styles:
...
| `/scores/stat-validation?fixtureId=...&seq=...&statKeys=1,2,...` | `validateStatV2` | Multi-stat validation with indexed predicates and multi-leg strategies. |
| `/scores/stat-validation-v3?fixtureId=...&seq=...&statKeys=1,2,...` | `validateStatV3` | Optimised multiproof validation with compressed proof data (v1.5.6+, up to 5 keys). |

For V2 and V3, the `statKeys` order is part of the proof contract. The response arrays `statsToProve` and `statProofs` (V2) or `leaves` (V3) are mapped by position, and strategy fields such as `index`, `indexA`, `indexB`, and `statIndex` refer to those same `0..N` positions.
...
For V3 multiproof validation, use the dedicated endpoint:

```typescript
const urlV3 = "/scores/stat-validation-v3?fixtureId=18218149&seq=1087&statKeys=1002,1007,2007";
const valV3 = (await users.apiClient.get(urlV3)).data;

const payloadV3 = {
  ts: new BN(valV3.summary.updateStats.minTimestamp),
  fixtureSummary,
  fixtureProof: mapProof(valV3.subTreeProof),
  mainTreeProof: mapProof(valV3.mainTreeProof),
  eventStatRoot: Array.from(valV3.eventStatRoot),
  leaves: valV3.statsToProve.map((leaf: any) => ({
    stat: leaf.stat,
    statProof: mapProof(leaf.statProof),
  })),
  leafIndices: valV3.multiproof.indices,
  multiproofHashes: mapProof(valV3.multiproof.hashes),
};
...
<Info>
The runnable scripts discover current `fixtureId` and `seq` values from bounded API windows. Optional `TXLINE_FIXTURE_ID` and `TXLINE_SEQ` overrides must be supplied together and should come from the same observed score record.
</Info>

...
This page uses the public `/api/scores/stat-validation` shape with `statKey` and optional `statKey2`. Use the same network for the API request, IDL/types, program ID, and on-chain root PDA.

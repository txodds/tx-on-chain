...
- You are using Node.js `20.18` or newer. The current lockfile includes Solana codec packages that require `node >=20.18.0`.
...
|---------|--------------|---------------|
| Example script fails before connecting to TxLINE | Node runtime or environment variables do not match the current examples | Use Node.js `20.18+`, set `ANCHOR_PROVIDER_URL`, `ANCHOR_WALLET`, and `TOKEN_MINT_ADDRESS`, then run from the repository root |
...
| Subscription broadcast/confirmation timed out and a recovery sidecar remains | The signed transaction may already have reached Solana | Keep the sidecar, rerun with the same wallet/network/tier context, and let the example query the saved public signature; never delete it merely to submit again |

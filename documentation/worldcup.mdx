...
Keep the confirmed `txSig` if activation fails. It is a public Solana transaction identifier and can be reused for activation-only recovery. After a `401`, obtain a fresh guest JWT and re-sign because the JWT is part of the message. Retry transient `5xx` responses, including `504`, only with bounded backoff. Do not send another `subscribe` transaction automatically, and do not retry or refresh the JWT in response to `400` or `403`.

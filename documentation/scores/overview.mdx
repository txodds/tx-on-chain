...
<CardGroup cols={2}>
  ...
  <Card title="Dual Environments" icon="layer-group">
    Identical data schemas available across MainNet (Live) and DevNet (Testing).
  </Card>
</CardGroup>

## Feed Lifecycle & Coverage

The feed tracks the complete state machine of a game, providing deep granularity across periods, halves, and operational phases:

- **Pre-Match:** Lineups, roster verification, and status changes.
- **In-Play:** Live state tracking, possession changes, and score updates.
- **Overtime:** Seamless state scaling for extra periods.
- **Post-Match:** Final validation and official settlement triggers.
